...
The winding is measured by :math:`k = \gamma \int G\,\mathrm{d}t`, the same
...
winding :math:`k`. Writing :math:`M^{+}(r) = M_x + iM_y` and taking its Fourier
...
:math:`\tilde F^{+}(0)`. This is the configuration-state treatment of echo
trains introduced by Hennig [1]_, in the notation of Weigel's review [2]_,
...
:math:`\tilde F^{+}(k)` and :math:`\tilde F^{-}(k)`. Longitudinal magnetization
is modulated across the voxel too -- a pulse can store a wound-up transverse
state along :math:`z`, where it neither dephases further nor decays with
:math:`T_2` -- so it has configurations of its own, :math:`\tilde Z(k)`. Those
...
.. figure:: /generated/figures/state_ladder.png
   ...
   :math:`\tilde F^{+}(0)` and :math:`\tilde F^{-}(0)` are complex conjugates
...
A hard pulse of flip angle :math:`\alpha` and phase :math:`\varphi` acts
...
.. figure:: /generated/figures/rf_operator.png
   ...
   The magnitude of :math:`\mathbf{T}` at three flip angles. A 180 degree pulse
   swaps the two transverse families outright -- perfect refocusing -- and
   inverts :math:`\tilde Z`. A 90 degree pulse splits every state three ways: it
   ...
   :math:`z`. A small pulse mostly leaves things where they were, which is why
...
.. figure:: /generated/figures/shift_operator.png
   ...
   one order in :math:`\tilde F^{+}` and down one in :math:`\tilde F^{-}`. The
...
Between events, transverse states decay by :math:`E_2 = e^{-\Delta t / T_2}`
and longitudinal states by :math:`E_1 = e^{-\Delta t / T_1}`. Recovery toward
equilibrium, though, is *spatially uniform*: it adds magnetization with no
modulation at all, so it enters :math:`\tilde Z(0)` alone.
...
.. figure:: /generated/figures/relaxation.png
   ...
   of longitudinal relaxation does to a set of :math:`\tilde Z` states: every
   order is scaled by :math:`E_1`, and only order zero gains the
   :math:`M_0(1 - E_1)` that regrowth supplies. A modulated longitudinal state
...
Plotting :math:`k` against time turns a sequence into a picture. Between pulses
every transverse pathway moves at a constant rate set by the gradients;
at each pulse it may continue, reverse, or be parked along :math:`z` until a
later pulse brings it back. **Wherever a transverse pathway crosses**
:math:`k = 0` **there is an echo**, and the sample is the sum of every pathway
...
.. figure:: /generated/figures/phase_graph.png
   ...
   pathways, dotted lines are magnetization parked along :math:`z` between
   ...
   or more intervals longitudinal, and so were spared :math:`T_2` decay while
   they were there.
...
spend part of their life along :math:`z`, so they arrive with less
:math:`T_2` decay than their echo time suggests, and the train picks up
...
.. figure:: /generated/figures/mono_exponential.png
   ...
   Left, a 48-echo train at three refocusing angles beside :math:`e^{-t/T_2}`.
   Right, the :math:`T_2` a mono-exponential fit of each train returns. Only the
...
one, so after :math:`n` shifts nothing beyond order :math:`n` exists, and
...
A state at order :math:`k` is wound up in space, and anything that moves spins
around inside the voxel scrambles that winding. Over an interval a state
accumulates a diffusion b-factor weighted by :math:`k^2` while it is
longitudinal, and by :math:`k^2 + k + 1/3` while it is transverse and winding
through the interval [2]_. Order zero is untouched, which is why diffusion enters
...
formalism that matter in practice, both from Malik et al [3]_: a **semisolid
pool**
...
:math:`T_2'` dephasing is not automatic -- what a balanced sequence needs is
...
[4]_ and is described in :doc:`implementation`.
